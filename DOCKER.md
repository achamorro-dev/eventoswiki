...
- **pnpm**: Usa pnpm en versión específica (11.23.0) gestionada por Corepack para instalación rápida y eficiente de dependencias
...
Este error se ha solucionado en el Dockerfile actual usando una versión específica de pnpm (11.23.0) vía Corepack (sincronizada con el campo `packageManager` de `package.json`) en lugar de `@latest` y configurando correctamente las variables de entorno de pnpm.
