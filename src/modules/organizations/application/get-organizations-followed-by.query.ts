...
export class GetOrganizationsFollowedByQuery extends Query<Organization[], { userId: string }> {
  ...
  execute({ userId }: { userId: string }): Promise<Organization[]> {
    return this.organizationRepository.findOrganizationsFollowedByUserId(userId)
  }
}
