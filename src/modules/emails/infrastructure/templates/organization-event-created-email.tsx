...
export function OrganizationEventCreatedEmail({
  ...
  const formattedTime = event.startsAt.toLocaleTimeString('es-ES', {
    hour: '2-digit',
    minute: '2-digit',
  })
  const endTime = event.endsAt.toLocaleTimeString('es-ES', {
    hour: '2-digit',
    minute: '2-digit',
  })

  const minPrice = event.getMinTicketPrice()
  const hasTickets = event.tickets.hasTickets()
  const priceText = minPrice === 0 ? 'Gratis' : `Desde ${minPrice?.toFixed(2)}€`

  return (
    <Html>
      <Head />
      <Preview>
        {organization.name} ha creado un nuevo evento: {event.title}
      </Preview>
      <Tailwind>
        <Body className="bg-gray-50 font-sans">
          <Container className="mx-auto max-w-2xl">
            <Section className="px-5">
              <Img src={event.image.toString()} alt={event.title} className="mb-6 w-full rounded-lg" />
            </Section>
            ...
            <Section className="px-5 py-8">
              <Container className="rounded-lg bg-white p-6 shadow-sm">
                ...
                <Section className="mb-5">
                  <Text className="mt-0 mb-2 font-bold text-gray-900 text-xl">{event.title}</Text>
                  <Text className="my-3 text-gray-700 text-sm leading-6">{event.shortDescription}</Text>
                </Section>
                ...
                {event.location && (
                  <Section className="mt-4">
                    ...
                    <Text className="my-2 text-gray-900 text-sm">{event.location}</Text>
                  </Section>
                )}

                {hasTickets && (
                  <Section className="mt-4">
                    <Text className="mb-1 font-bold text-gray-600 text-xs">🎫 Precio</Text>
                    <Text className="my-2 font-bold text-gray-900 text-sm">{priceText}</Text>
                  </Section>
                )}
