        ...
        <div className="space-y-6 py-4">
          <div className="flex w-full flex-col items-start gap-4 lg:flex-row">
            ...
            <div className="lg:max-w-1/3 flex w-full flex-col items-center gap-2">
              <FormField control={form.control} name="image" render={({ field }) => (
                <>
                  {field.value ? (
                    <img src={field.value} alt={form.watch('title')} className="h-72 rounded-md border-transparent object-cover" />
                  ) : (
                    <div aria-invalid={!!form.formState.errors['image']} className={'aria-invalid:border-destructive border-input bg-input flex w-full items-center justify-center rounded-md border-2'}>
                      <CameraSlash className="h-72 w-48 text-gray-400" />
                    </div>
                  )}
                  <Button variant="outline" type="button" size="icon" className="z-1 -mt-7 ml-4" disabled={isLoading} onClick={() => fileInputRef.current?.click()}>
            ...
            <div className="w-full flex-1 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <FormField control={form.control} name="shortDescription" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea id="shortDescription" placeholder="Describe el evento..." className="min-h-[100px]" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <FormField control={form.control} name="startsAt" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <DateTimePicker disabled={form.formState.isSubmitting} placeholder="Elige una fecha" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  ...
                  <FormField control={form.control} name="endsAt" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <DateTimePicker value={field.value} onChange={field.onChange} disabled={form.formState.isSubmitting} placeholder="Elige una fecha" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
              ...
              <div className="border-t pt-6">
                ...
                <div className="space-y-4">
                    ...
                    <>
                      <FormField control={form.control} name="location" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <ProvinceSelect id="location" placeholder="Selecciona una provincia" provinces={provinces} className="w-full" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={form.control} name="place" render={({ field }) => (
                        <>
                          <FormItem>
                            ...
                            <FormControl>
                              <PlaceSearch className="w-full" value={field.value as Primitives<Place> | undefined} onPlaceSelect={(place: Primitives<Place>) => { form.setValue('place', place) }} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                  ...
                  {(type === MeetupTypes.Online || type === MeetupTypes.Hybrid) && (
                    <FormField control={form.control} name="streamingUrl" render={({ field }) => (
                      <FormItem>
                        <FormLabel htmlFor="streamingUrl">URL del streaming</FormLabel>
                        <FormControl>
                          <Input id="streamingUrl" type="url" placeholder="https://..." className="w-full" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
              ...
              <div className="border-t pt-6">
                ...
                <div className="space-y-4">
                  <FormField control={form.control} name="allowsAttendees" render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                      <div className="space-y-0.5">
                        <FormLabel htmlFor="allowsAttendees">Permitir registro de asistentes</FormLabel>
                        <p className="text-sm text-muted-foreground">Los usuarios podrán registrarse como asistentes a este meetup</p>
                      </div>
                      <FormControl>
                        <Switch id="allowsAttendees" checked={field.value} onCheckedChange={field.onChange} />
                      </FormControl>
                    </FormItem>
                  )} />
                  {form.watch('allowsAttendees') && (
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      <FormField control={form.control} name="registrationEndsAt" render={({ field }) => (
                        <FormItem>
                          <FormLabel htmlFor="registrationEndsAt">Fecha límite de registro</FormLabel>
                          <p className="text-sm text-muted-foreground mb-2">Los usuarios no podrán registrarse después de esta fecha</p>
                          <FormControl>
                            <DateTimePicker disabled={form.formState.isSubmitting} placeholder="Elige una fecha (opcional)" value={field.value} onChange={field.onChange} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={form.control} name="maxAttendees" render={({ field }) => (
                        <FormItem>
                          ...
                          <p className="text-sm text-muted-foreground mb-2">Número máximo de asistentes permitidos</p>
                          <FormControl>
                            <Input id="maxAttendees" type="number" min="1" placeholder="Ilimitado" {...field} value={field.value ?? ''} onChange={(e) => { const value = e.target.value; field.onChange(value === '' ? undefined : Number(value)) }} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
              ...
              <div className="border-t pt-6">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <FormField control={form.control} name="tagColor" render={({ field }) => (
                      <FormItem>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {form.watch('tags')?.map((tag, index) => (
                      <Badge key={index} className="text-white" style={{ backgroundColor: form.watch('tagColor') }}>
                        {tag}
                        <Button type="button" variant="ghost" size="icon" className="h-4 w-4 p-0 hover:cursor-pointer hover:bg-transparent" onClick={() => onRemoveTag(index)}>
                          <X className="h-3 w-3" />
          ...
          <div className="border-t pt-6">
            ...
            <FormField control={form.control} name="content" render={({ field }) => (
              <FormItem>
                <FormControl>
                  <RichEditor content={field.value} onContentChange={field.onChange} onUploadImage={uploadImageForEditor} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )} />
